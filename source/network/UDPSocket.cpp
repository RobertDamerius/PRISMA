...
#define NETWORK_ADDRESS_PORT(x)   x.sin_port
#ifdef _WIN32
#define NETWORK_ADDRESS_IP(x)     x.sin_addr.S_un.S_addr
#else
#define NETWORK_ADDRESS_IP(x)     x.sin_addr.s_addr
#endif
...
int32_t UDPSocket::SendTo(IPAddress destination, const uint8_t *bytes, int32_t size){
    sockaddr_in addr;
    addr.sin_family = AF_INET;
    memset((void*) &addr.sin_zero[0], 0, 8);
    NETWORK_ADDRESS_PORT(addr) = htons(destination.port);
    NETWORK_ADDRESS_IP(addr) = htonl((0xFF000000 & (destination.ip[0] << 24)) | (0x00FF0000 & (destination.ip[1] << 16)) | (0x0000FF00 & (destination.ip[2] << 8)) | (0x000000FF & destination.ip[3]));
...
int32_t UDPSocket::ReceiveFrom(IPAddress& source, uint8_t *bytes, int32_t maxSize){
    sockaddr_in addr{};
    #ifndef _WIN32
    ...
    uint32_t u32 = ntohl(NETWORK_ADDRESS_IP(addr));
    ...
    source.port = (uint16_t)ntohs(NETWORK_ADDRESS_PORT(addr));
