...
class ShaderMinimalEnvironment: protected Shader {
    public:
        using Shader::Use;

        /**
         * @brief Construct a minimal environment shader.
         */
        ShaderMinimalEnvironment(): locationAmbientLightColor(0), locationNumDirectionalLights(0), locationNumPointLights(0), locationNumSpotLights(0), locationNumRayTracingSteps(0), locationAmbientOcclusionEnhance(0), locationShadowBiasMinMax(0), maxNumDirectionalLights(0), maxNumPointLights(0), maxNumSpotLights(0), numShadowCascades(0) {}
        ...
        bool Generate(GLuint maxNumDirectionalLights, GLuint maxNumPointLights, GLuint maxNumSpotLights, GLuint numShadowCascades){
            ...
            if(!Shader::Generate(FileName(FILENAME_SHADER_MINIMAL_ENVIRONMENT), replacement)){
